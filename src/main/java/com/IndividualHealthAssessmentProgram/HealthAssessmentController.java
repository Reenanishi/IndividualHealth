...
public class HealthAssessmentController implements Initializable {
    ...
    @FXML
    private void clearFieldAndLabelAction(ActionEvent event) {

        try {
            BloodPressureField.setText("");
            BloodPressureStatusLabel.setText("");
            BMIField.setText("");
            BMIStatusLabel.setText("");
            GlucoseBloodSugarField.setText("");
            GlucoseBloodSugarStatusLabel.setText("");
            TriglyceridesField.setText("");
            TriglyceridesStatusLabel.setText("");
            CholesterolLDLField.setText("");
            CholesterolLDLStatusLabel.setText("");
            CholesterolHDLField.setText("");
            CholesterolHDLStatusLabel.setText("");
            CholesterolTotalStatusLabel.setText("");
        } catch (Exception ex) {
    ...
    private void saveToFile(ActionEvent event) {
        try{
            ...
            JAXBContext jContext = JAXBContext.newInstance(new Class[] {HealthReport.class});
            ...
            healthReport.setBloodPressure(bloodPressure);
            healthReport.setBMI(bmi);
            healthReport.setBloodGlucoseHemoglobin(bloodGlucoseHemoglobin);
            healthReport.setCholesterolHDL(cholesterolHDL);
            healthReport.setCholesterolLDL(cholesterolLDL);
            healthReport.seTriglycerides(triglycerides);
    ...
    private void loadFromFile(ActionEvent event) {
        try{
            ...
            if (!file.exists()) {
                dialogHandler("File do not exists");
                return;
            }
            //creating the JAXB context
            JAXBContext jContext = JAXBContext.newInstance(new Class[] {HealthReport.class});
